...
\newcommand{\thesistitle}{Translation Operators for Translation-Variant Modal Green's Functions in the Black Box Kernel-Independent Fast Multipole Method}
...
Section 1 is devoted to the FMM

Section 1a is devoted to FMM-accelereated fast solvers

Section 2 is devoted to the KIFMM

Section 3 KIFMM with modal green's functions

bbFMM with modal green's functions
...
\begin{align}
K(\mathbf{x},\mathbf{x'})=\sum_{n\in\mathbb{Z}} \frac{e^{in(\theta-\theta')}}{\sqrt{2\pi}}s_n(r,z,r',z')
\end{align}

for $\mathbf{x}=(r,z,\theta)$ and $\mathbf{x'}=(r',z',\theta')$, where

\begin{align}
s_n(r,z,r',z')=\frac{1}{\sqrt{8\pi^3 rr'}}\mathbf{Q}_{n-\frac{1}{2}}\bigg(\frac{r^2+(r')^2+(z-z')^2}{2rr'}\bigg)
\end{align}

In particular, we are looking at the Fourier modes of the single-layer three-dimensional Laplace kernel:

\begin{align}
\frac{1}{4\pi |\mathbf{x}-\mathbf{x'}|}=\sum_{n\in\mathbb{Z}} \frac{e^{in(\theta-\theta')}}{\sqrt{2\pi}} k_n(r,z,r',z')
\end{align}
